...
class DeltaEGraph(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Add quality grade regions (4단계 품질 등급 영역)
        # 1. 우수 등급 영역 (DE ≤ 1.0) - 진한 녹색
        ...
        # Add quality grade threshold lines (품질 등급 구분선)
        # 우수-양호 구분선 (DE = 1.0)
        ...
        # 허용-불량 구분선 (DE = 3.0) - 중요한 불량 판정 기준선
    ...
    def toggle_view_mode(self):
        ...
        if self.heatmap_mode:
            ...
            # Create heatmap visualization (품질 등급별 색상)
            if len(self.times) > 0:
                colors = []
    ...
    def add_point(self, de_value: float) -> bool:
        ...
        # Check if DE value exceeds quality thresholds (품질 등급 판정)
        grade = gb.get_quality_grade(de_value)
        # 불량 이상일 경우 경고 반환 (불량: defective, 범위초과: out_of_range)
...
class ColorAnalyzerTab(QWidget):
        ...
        try:
            ...
            for key, label in self.result_labels.items():
                if key in color_diffs:
                    ...
                    # Quality grade based color coding (품질 등급별 색상 코딩)
            ...
            # Update status (품질 등급별 상태 메시지)
            de_value = color_diffs['de']
            grade = gb.get_quality_grade(de_value)
            
            # 품질 등급별 상태 메시지 및 색상 설정
    ...
    def show_warning_message(self, de_value: float):
        """Display quality grade based warning message (품질 등급별 경고 메시지)"""
        try:
            grade = gb.get_quality_grade(de_value)
            ...
            if grade == 'defective':
                ...
                msg.setWindowTitle("불량 품질 경고")
                msg.setText(f"색차값이 불량 등급입니다: DE = {de_value:.2f}")
                msg.setInformativeText(f"불량 기준: DE > ±{gb.QUALITY_GRADE_ACCEPTABLE}\n제품 품질을 즉시 확인해 주세요.")
            elif grade == 'out_of_range':
                msg.setIcon(QMessageBox.Critical)
                msg.setWindowTitle("색차 범위 초과")
                msg.setText(f"색차값이 측정 범위를 초과했습니다: DE = {de_value:.2f}")
                msg.setInformativeText(f"측정 범위: ±{gb.QUALITY_GRADE_DEFECTIVE}\n시스템을 즉시 점검해 주세요.")
            else:
                # Should not happen, but just in case
                msg.setIcon(QMessageBox.Information)
                msg.setWindowTitle("색차 알림")
                msg.setText(f"색차값: DE = {de_value:.2f}")
                msg.setInformativeText("품질 상태를 확인해 주세요.")
    ...
    def process_frames(self, color_image):
        ...
        try:
            ...
            if self.current_analysis:
                ...
                # 품질 등급별 샘플링 영역 색상
                if grade == 'excellent':
                    rect_color = (0, 200, 0)  # 진한 녹색
                elif grade == 'good':
                    rect_color = (0, 255, 0)  # 밝은 녹색
                elif grade == 'acceptable':
                    rect_color = (0, 255, 255)  # 노란색
                elif grade == 'defective':
                    rect_color = (0, 0, 255)  # 빨간색
                else:  # out_of_range
                    rect_color = (128, 0, 128)  # 보라색
